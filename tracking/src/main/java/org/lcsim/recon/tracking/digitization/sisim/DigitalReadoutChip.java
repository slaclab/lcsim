...
/**
 * Digital readout chip class.  This class supports the minimal functions
 * expected of a digital readout chip.  The charge on a strip/pixel is digitized
 * to give a specified ADC value if the charge is above the noise threshold.
 * Noise is added to strips/pixels with charge, and random noise hits are generated
 * as well.  Methods are provided to decode the charge and time (although the
 * current implementation always returns a time of 0).
 *
...
public class DigitalReadoutChip implements ReadoutChip {
    ...
    public void setNoiseThreshold(double noise_threshold) {
        ...
        _channel.setNoiseThreshold(noise_threshold);
    }
    ...
    public void setConversionConstant(int adc_for_hit) {
        _channel.setConversionConstant(adc_for_hit);
    }

    /**
     * Return the DigitalChannel associated with a given channel number.
     * For the digital readout, there is a single instance of DigitalChannel
     * and thus the channel number is ignored.
     *
     * @param channel_number channel number
     * @return associated DigitalChannel
     */
    public DigitalChannel getChannel(int channel_number) {
        return _channel;
    }
    ...
    /**
     * Decode the hit charge stored in the RawTrackerHit.  Since this method
     * isn't well defined for digital readout, return the noise threshold if
     * there is a hit.
     *
     * @param hit raw hit
     * @return hit pseudo-charge in units of electrons
     */
    public double decodeCharge(RawTrackerHit hit) {
        ...
        //  If there is a hit (i.e., adc > 0) return the charge as being the
        //  value of the noise threshold, otherwise return 0
        if (adc > 0) return _noise_threshold;
        else return 0.;
    }

    /**
     * Decode the hit time.  Currently, the digital readout chip ignores the
    ...
    private SortedMap<Integer, List<Integer>> digitize(SiElectrodeDataCollection data,
        ...
        for (Integer channel : data.keySet()) {
            ...
            //  Create a list containing the adc value - for the digital readout
     ...
     */
    private class DigitalChannel implements ReadoutChannel {

        private double _noise_intercept = 0.;
        private double _noise_slope = 0.;
        private double _noise_threshold = 100.;
        private int _adc_for_hit = 100;
         ...
         * @param noise_threshold
         */
        private void setNoiseThreshold(double noise_threshold) {
            _noise_threshold = noise_threshold;
        }
        ...
        private int computeAdcValue(double charge) {
            ...
            //  Set to the specified value for a hit if we are above the
            //  noise threshold
            int adc = 0;
            if (charge > _noise_threshold) adc = _adc_for_hit;
